...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum_KickSoccer.app"
    exit -127
fi
...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-kicksoccer-macos
	cd /tmp/electrum-kicksoccer-macos
...
rm -rf /tmp/electrum-kicksoccer-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-kicksoccer-macos/image/
cp -r $1 /tmp/electrum-kicksoccer-macos/image/
...
${genisoimage} \
    ...
    -l \
    -probe \
    -V "Electrum-KickSoccer" \
    -no-pad \
    -r \
    -dir-mode 0755 \
    -apple \
    -o Electrum-kicksoccer_uncompressed.dmg \
    /tmp/electrum-kicksoccer-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-kicksoccer_uncompressed.dmg electrum-kicksoccer-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-kicksoccer_uncompressed.dmg

echo "Done."
sha256sum electrum-kicksoccer-$VERSION.dmg
