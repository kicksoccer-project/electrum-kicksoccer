...
2. Build image
    ...
    ```
    $ sudo docker build -t electrum-kicksoccer-wine-builder-img contrib/build-wine/docker
    ```
...
3. Build Windows binaries
    ...
    $ FRESH_CLONE=contrib/build-wine/fresh_clone && \
        ...
        mkdir -p $FRESH_CLONE && \
        cd $FRESH_CLONE  && \
        git clone https://github.com/kicksoccer-project/electrum-kicksoccer.git && \
    ...
    $ git checkout $REV
    $ sudo docker run -it \
        --name electrum-kicksoccer-wine-builder-cont \
        -v $PWD:/opt/wine64/drive_c/electrum-kicksoccer \
        --rm \
        --workdir /opt/wine64/drive_c/electrum-kicksoccer/contrib/build-wine \
        electrum-kicksoccer-wine-builder-img \
        ./build.sh
    ```
...
Electrum-KickSoccer Windows builds are signed with a Microsoft Authenticode™ code signing
certificate in addition to the GPG-based signatures.

The advantage of using Authenticode is that Electrum-KickSoccer users won't receive a 
