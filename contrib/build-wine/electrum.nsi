...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-KickSoccer"
  !define PRODUCT_WEB_SITE "https://github.com/kicksoccer-project/electrum-kicksoccer"
  !define PRODUCT_PUBLISHER "KickSoccer"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-kicksoccer-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-kicksoccer\electrum\gui\icons\electrum.ico"
...
Section
  ...
  File "c:\electrum-kicksoccer\electrum\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-kicksoccer-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-kicksoccer-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-kicksoccer-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-kicksoccer-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-kicksoccer-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum-kicksoccer
  WriteRegStr HKCU "Software\Classes\kicksoccer" "" "URL:ksoc Protocol"
  WriteRegStr HKCU "Software\Classes\kicksoccer" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\kicksoccer" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\kicksoccer\shell\open\command" "" "$\"$INSTDIR\electrum-kicksoccer-${PRODUCT_VERSION}.exe$\" $\"%1$\""
